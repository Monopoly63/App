...
var XYZ_TO_LMS, LINEAR_P3_TO_XYZ, LINEAR_SRGB_TO_XYZ, LINEAR_P3_TO_LMS, LMS_TO_LINEAR_P3, XYZ_TO_LINEAR_SRGB, LINEAR_P3_TO_LINEAR_SRGB, LINEAR_SRGB_TO_LINEAR_P3, SRGB_LUMA, GAMUT_EPS;
var init_color_space = __esm({
  "src/color-space.ts"() {
    ...
    LINEAR_P3_TO_LMS = mat3Mul(XYZ_TO_LMS, LINEAR_P3_TO_XYZ);
    LMS_TO_LINEAR_P3 = mat3Inv(LINEAR_P3_TO_LMS);
    XYZ_TO_LINEAR_SRGB = mat3Inv(LINEAR_SRGB_TO_XYZ);
    LINEAR_P3_TO_LINEAR_SRGB = mat3Mul(XYZ_TO_LINEAR_SRGB, LINEAR_P3_TO_XYZ);
    LINEAR_SRGB_TO_LINEAR_P3 = mat3Inv(LINEAR_P3_TO_LINEAR_SRGB);
    SRGB_LUMA = [0.2126, 0.7152, 0.0722];
    GAMUT_EPS = 1e-3;
  }
});
...
async function getPalette(source, options) {
  ...
  if (options?.worker) {
    ...
    if (isWorkerSupported2()) {
      const pixels2 = await loadPixels(source, options);
      const { createPixelArray: createPixelArray2, resolveOutputGamut: resolveOutputGamut2 } = await Promise.resolve().then(() => (init_pipeline(), pipeline_exports));
      const pixelArray = createPixelArray2(pixels2.data, pixels2.width * pixels2.height, opts.quality, {
        ignoreWhite: opts.ignoreWhite,
        whiteThreshold: opts.whiteThreshold,
        alphaThreshold: opts.alphaThreshold,
        minSaturation: opts.minSaturation
      });
      const nativeGamut = pixels2.colorSpace ?? "srgb";
      const outputGamut = resolveOutputGamut2(pixelArray, nativeGamut, opts.gamut);
      return extractInWorker2(
        pixelArray,
        opts.colorCount,
        options?.signal,
        nativeGamut,
